...
addBtn.addEventListener("click", function (e) {
  ...
  important = false;
  document
    .getElementById("important-icon")
    .setAttribute("src", "icons/starGrey.png");
  // console.log(notesObj)
});
...
function showNotes() {
  ...
  notesObj.forEach(function ([title, text, important], index) {
    html += `
                <div class= "${
                  important ? "border-3 border-danger" : " "
                } card mx-2 my-2 noteCard " style = "width: 18rem;}">
                <div class="${index} card-body d-flex flex-column">
                  <h5 id = "title${index}" class="card-title flex-start">${
      title == "" ? "Note " + index + 1 : title
    }</h5>
                  <p id = "text${index}" class="card-text flex-grow-1">${text}</p>
                  <div class="d-flex gap-2 flex-end">
                  <button id = "${index}" onclick = "deleteNote(this.id)" class="btn btn-sm btn-outline-danger" ><i class="bi bi-trash3"></i></button>
                  <button id = "${index}" onclick = "editNote(this.id)" class="btn btn-sm btn-outline-primary type = "button" data-bs-toggle="modal" data-bs-target="#staticBackdrop"><i class="bi bi-pencil-square"></i> </button>
                  <button id = "${index}" onclick = "deleteNote(this.id)" class="btn btn-sm btn-outline-success" ><i class="bi bi-check2-square"></i> Mark as done</button>
                  </div>
                </div>
                </div>
                        `;
...
function switchImportant() {
  if (important) {
    ...
    document
      .getElementById("important-icon")
      .setAttribute("src", "icons/starOrange.png");
    important = true;
  }
}
